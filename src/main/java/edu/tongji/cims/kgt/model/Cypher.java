...
public class Cypher {
    ...
    // 查询语句参数名称与QueryProp的参数名称对应
    public final static String CONTAIN_NODE = "match (n:node{name: {props}.name}) return n";
...
//        return "match p=(from:node{name:{props}.name})-[rel*1.." + degree + "]-(to:node) where all(x in rels(p) where x.name <> '实例') RETURN p,extract(x IN rels(p)| startnode(x)) as dir";
        return "match p=(from:node{name:{props}.name})-[rel*1.." + degree + "]-(to:node) return p,extract(x IN rels(p)| startnode(x)) as dir";
    }
